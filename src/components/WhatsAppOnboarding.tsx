...
export function WhatsAppOnboarding({ user, onComplete, onSkip }: WhatsAppOnboardingProps) {
  ...
  if (checking) {
    return (
      <div className="min-h-[100dvh] bg-[#111b21] text-[#e9edef] flex items-center justify-center">
  ...
  return (
    <div className="min-h-[100dvh] bg-[#111b21] text-[#e9edef] flex flex-col">
      {/* Header */}
      <div className="h-[60px] bg-[#202c33] px-4 flex items-center gap-3 flex-shrink-0">
        <div className="w-8 h-8 rounded-full bg-[#00a884] flex items-center justify-center">
          <Smartphone className="w-4 h-4 text-black" />
        </div>
        <h1 className="text-lg font-semibold">WhatsApp Setup</h1>
      </div>

      {/* Step indicator */}
      <div className="flex items-center justify-center gap-2 px-4 py-4 bg-[#0b141a]">
        {['pair', 'permissions', 'location'].map((s, i) => (
          <div key={s} className="flex items-center gap-2">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
              ...
              {['pair', 'permissions', 'location'].indexOf(step) > i ? <Check className="w-4 h-4" /> : i + 1}
            </div>
            <span className={`text-xs ${step === s ? 'text-[#e9edef] font-medium' : 'text-[#8696a0]'}`}>
              {s === 'pair' ? 'Link WhatsApp' : s === 'permissions' ? 'Permissions' : 'Location'}
            </span>
            {i < 2 && <div className="w-8 h-px bg-[#222d34]" />}
          </div>
        ))}
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {step === 'pair' && (
          <div className="max-w-lg mx-auto w-full p-6 space-y-6">
            <div className="text-center">
              <Smartphone className="w-16 h-16 text-[#00a884] mx-auto mb-4" />
              <h2 className="text-xl font-semibold mb-2">Link Your WhatsApp</h2>
              <p className="text-sm text-[#8696a0]">Connect your WhatsApp to use it from this web app</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => setPairMethod('qr')}
                ...
                <QrCode className={`w-10 h-10 ${pairMethod === 'qr' ? 'text-[#00a884]' : 'text-[#8696a0]'}`} />
                <span className={`font-medium text-sm ${pairMethod === 'qr' ? 'text-[#00a884]' : ''}`}>Scan QR Code</span>
                <span className="text-xs text-[#8696a0] text-center">Use your phone to scan</span>
              </button>
              <button onClick={() => setPairMethod('phone')}
                className={`flex flex-col items-center gap-3 rounded-xl border p-6 transition ${
                  pairMethod === 'phone' ? 'border-[#00a884] bg-[#00a884]/10' : 'border-[#222d34] bg-[#202c33] hover:bg-[#2a3942]'
                }`}>
                <Phone className={`w-10 h-10 ${pairMethod === 'phone' ? 'text-[#00a884]' : 'text-[#8696a0]'}`} />
                <span className={`font-medium text-sm ${pairMethod === 'phone' ? 'text-[#00a884]' : ''}`}>Link with Phone Number</span>
                <span className="text-xs text-[#8696a0] text-center">Enter your phone number</span>
              </button>
            </div>
            ...
            {pairMethod === 'phone' && (
              <div className="space-y-3">
                <label className="text-sm text-[#8696a0]">Phone number with country code</label>
                <input type="tel" placeholder="e.g. 32470123456"
                  value={phoneNumber} onChange={e => setPhoneNumber(e.target.value.replace(/\D/g, ''))}
                  className="w-full bg-[#202c33] border border-[#222d34] rounded-lg px-4 py-3 text-sm outline-none focus:border-[#00a884] text-[#e9edef] placeholder-[#8696a0]" />
              </div>
            )}

            {/* QR Code display */}
            {pairMethod === 'qr' && qrCode && (
              <div className="flex flex-col items-center gap-3 bg-[#202c33] rounded-xl p-6">
                <img src={qrCode} alt="QR" className="w-48 h-48 rounded-lg bg-white p-2" />
                <p className="text-xs text-[#8696a0] text-center">Open WhatsApp {'>'} Linked Devices {'>'} Link a Device</p>
              </div>
            )}

            {/* Pairing code display */}
            {pairingCode && (
              <div className="bg-[#202c33] rounded-xl p-6 text-center">
                <p className="text-sm text-[#8696a0] mb-2">Your pairing code:</p>
                <p className="text-2xl font-mono font-bold tracking-widest text-[#00a884]">{pairingCode}</p>
                <p className="text-xs text-[#8696a0] mt-2">Enter this code in WhatsApp {'>'} Linked Devices</p>
              </div>
            )}
            ...
            <div className="flex gap-3">
              <button onClick={() => setPairing(true) || handleStartPair()}
                ...
                className="flex-1 bg-[#00a884] text-black font-semibold rounded-lg py-3 hover:bg-[#06cf9c] disabled:opacity-50 text-sm">
                {pairing ? (
                  <span className="flex items-center justify-center gap-2"><Loader2 className="w-4 h-4 animate-spin" /> Pairing...</span>
                ) : 'Link This Device'}
              </button>
            </div>
            ...
            {waStatus === 'paired' && (
              <div className="bg-[#00a884]/10 border border-[#00a884]/30 rounded-lg p-4 text-sm text-center">
            ...
            }} className="w-full text-sm text-[#8696a0] hover:text-[#e9edef] py-2">
              Skip for now
            </button>
          </div>
        )}
        ...
        {step === 'permissions' && (
          <div className="max-w-lg mx-auto w-full p-6 space-y-6">
            <div className="text-center">
              <ShieldCheck className="w-16 h-16 text-[#00a884] mx-auto mb-4" />
              <h2 className="text-xl font-semibold mb-2">WhatsApp Permissions</h2>
              <p className="text-sm text-[#8696a0]">Choose what Beatrice can do with your WhatsApp</p>
            </div>
            ...
            <div className="space-y-2">
              {allPermissions.map(p => (
                <button key={p.key} onClick={() => setPermissions(prev => ({ ...prev, [p.key]: !prev[p.key] }))}
                  ...
                  <div className={`w-10 h-5 rounded-full p-0.5 transition ${permissions[p.key] ? 'bg-[#00a884]' : 'bg-[#374045]'}`}>
                    <div className={`w-4 h-4 rounded-full bg-white transition ${permissions[p.key] ? 'translate-x-5' : ''}`} />
                  </div>
                </button>
              ))}
            </div>
            ...
            <div className="flex gap-3">
              <button onClick={handleSavePermissions} disabled={saving}
                className="flex-1 bg-[#00a884] text-black font-semibold rounded-lg py-3 hover:bg-[#06cf9c] disabled:opacity-50 text-sm">
                {saving ? <span className="flex items-center justify-center gap-2"><Loader2 className="w-4 h-4 animate-spin" /> Saving...</span> : 'Save WA Settings'}
              </button>
            </div>
          </div>
        )}
          ...
          <div className="max-w-lg mx-auto w-full p-6 space-y-6">
            <div className="text-center">
              <div className="w-16 h-16 rounded-full bg-[#00a884]/20 flex items-center justify-center mx-auto mb-4">
                <svg className="w-8 h-8 text-[#00a884]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
              </div>
              <h2 className="text-xl font-semibold mb-2">Location Access</h2>
              <p className="text-sm text-[#8696a0] mb-6">Beatrice uses your location to provide local information</p>
            </div>
            <button onClick={handleLocation}
              className="w-full bg-[#00a884] text-black font-semibold rounded-lg py-3 hover:bg-[#06cf9c] text-sm">
